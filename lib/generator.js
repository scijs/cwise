...
function outerFill(matched, order, procedure) {
  ...
  //Initiaize variables
  code.push("var i=0,l=0,v=0,o=0,d=0,sp="+frame_size)

  //Push initial frame
  for(i=0; i<dimension; ++i) {
    code.push("STACK["+i+"]=shape["+i+"]")
  }
  for(i=0; i<nargs; ++i) {
    code.push("STACK["+(i+static_args)+"]=ptr"+i)
  }
  if(has_index) {
    for(i=0; i<dimension; ++i) {
      code.push("STACK["+(i+index_start)+"]=0")
    }
  }
...
matched_loop:
  ...
  console.log("Creating procedure:", arglist.slice(0,-1), arglist[arglist.length-1])
