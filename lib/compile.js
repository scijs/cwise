...
function innerFill(order, proc, body) {
  ...
  for(j=0; j<nargs; ++j) {
    for(i=0; i<dimension; ++i) {
      ...
      idx = order[i]
      if(i === 0) {
        vars.push(["d",j,"s",i,"=t",j,"s",idx,"|0"].join(""))
      } else {
        vars.push(["d",j,"s",i,"=(t",j,"s",idx,"-s",pidx,"*t",j,"s",pidx,")|0"].join(""))
      }
    }
  }
  ...
  for(i=dimension-1; i>=0; --i) {
    idx = order[i]
    code.push(["for(i",i,"=s",idx,"|0;--i",i,">=0;){"].join(""))
  }
...
function outerFill(matched, order, proc, body) {
  ...
  for(var i=0; i<nargs; ++i) {
    ...
    for(var j=matched; j<dimension; ++j) {
      indexStr.push(["j",j,"*t",i,"s",order[j]].join(""))
    }
    code.push(["p",i,"=(",indexStr.join("+"),")|0"].join(""))
  }
...
function generateCWiseOp(proc, typesig) {
  ...
  for(var i=0; i<proc.arrayArgs.length; ++i) {
    ...
    arglist.push("p" + i)
    for(var j=0; j<dimension; ++j) {
      vars.push(["t",i,"s",j,"=t",i,"[",j,"]|0"].join(""))
    }
  ...
  if(proc.shapeArgs.length > 0) {
    vars.push("inline_shape=shape.slice(0)")
  }
